#include "transport_intracellular.h"

#include <sstream>
#include <iostream>

transportIntracellular::transportIntracellular() : Intracellular()
{
	intracellular_type = "transport_model";
    std::cout << "====== " << __FUNCTION__ << "() intracellular_type=" << intracellular_type << std::endl;
	parameters.clear();
}

// constructor using XML node
transportIntracellular::transportIntracellular(pugi::xml_node& node)
{
...
// Intracellular* transportIntracellular::clone() // --> 'Intracellular' does not name a type
// {
// 	return static_cast<Intracellular*>(new transportIntracellular(this));
// }

// rwh: review this
transportIntracellular::transportIntracellular(transportIntracellular* copy) 
{
...
void transportIntracellular::initialize_intracellular_from_pugixml(pugi::xml_node& node)
{
	pugi::xml_node node_transport_model = node.child( "transport" );
	if ( node_transport_model )
	{ 
        string transport_type = PhysiCell::xml_get_my_string_value (node_transport_model); 
        std::cout << "\n------------- "  << __FUNCTION__ << ": transport type = " << transport_type << std::endl;
    }
	
	pugi::xml_node node_species = node.child( "substrate" );
	while( node_species )
	{
        // ---------  substrate
        
		std::string substrate_name = node_species.attribute( "substrate_name" ).value(); 
		if( substrate_name != "" )
		{
...
void transportIntracellular::start()
{
...
double transportIntracellular::get_parameter_value(std::string param_name)
{
    rrc::RRVectorPtr vptr;
...
void transportIntracellular::set_parameter_value(std::string species_name, double value)
{
    rrc::RRVectorPtr vptr;
...
transportIntracellular* getRoadRunnerModel(PhysiCell::Phenotype& phenotype) {
	return static_cast<transportIntracellular*>(phenotype.intracellular);
}

void transportIntracellular::save(std::string path, std::string index)
{
...
std::string transportIntracellular::get_state()
{
    return sbml_filename;
}


int transportIntracellular::update_phenotype_parameters(PhysiCell::Phenotype& phenotype)
{
    for(auto elm : phenotype_species)
    {
        // motility params
...
int transportIntracellular::validate_PhysiCell_tokens(PhysiCell::Phenotype& phenotype)
{
    for(auto elm : phenotype_species)
    {
...
int transportIntracellular::validate_SBML_species()
{
...
int transportIntracellular::create_custom_data_for_SBML(PhysiCell::Phenotype& phenotype)
{
